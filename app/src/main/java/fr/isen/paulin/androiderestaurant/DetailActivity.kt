...
fun DishDetailScreen(dish: Items) {
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        Column(modifier = Modifier.padding(16.dp)) {
            Text(text = dish.nameFr ?: "Détails du plat", modifier = Modifier.padding(bottom = 16.dp))
            ImageCarousel(images = dish.images)
            Spacer(modifier = Modifier.height(16.dp))
            IngredientList(ingredients = dish.ingredients)
            Spacer(modifier = Modifier.height(16.dp))
            QuantityCounter(quantity = quantity.value, onIncrease = { if (quantity.value < 99) quantity.value++ }, onDecrease = { if (quantity.value > 1) quantity.value-- })
        }
...
fun QuantityCounter(quantity: Int, onIncrease: () -> Unit, onDecrease: () -> Unit) {
    Row(modifier = Modifier.padding(16.dp)) {
        Button(onClick = onDecrease, modifier = Modifier.size(48.dp)) { Text("-") }
        Spacer(modifier = Modifier.width(16.dp))
        Text(text = quantity.toString(), modifier = Modifier.align(Alignment.CenterVertically))
        Spacer(modifier = Modifier.width(16.dp))
        Button(onClick = onIncrease, modifier = Modifier.size(48.dp)) { Text("+") }
    }
}
